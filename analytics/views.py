...
class AnalyticsView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        serializer = SellerAnalyticsSerializer if request.user.is_verified_seller else SiteAnalyticsSerializer
        object = SellerAnalytics.objects.filter(seller=request.user) if request.user.is_verified_seller else SiteAnalytics.objects

        newest = object.order_by("created_at").reverse()[:48]
        return Response({
            "requested_at": timezone.now(),
            "last_update": newest[0].created_at if newest else None,
            "data": serializer(newest, many=True).data[::-1]
        })
